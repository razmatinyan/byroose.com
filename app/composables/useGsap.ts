...
>;
type PluginExport<K extends LoadablePlugin> = PluginModule<K>[K &
...
export function useGsap() {
   ...
   function createContext(setup: gsap.ContextFunc, scope?: AnimationScope) {
      ...
      const nextContext = gsapInstance.context(
         setup,
         scope ? (toValue(scope) ?? undefined) : undefined,
      );
   ...
   function createMatchMedia(
      ...
      const nextMedia = gsapInstance.matchMedia(
         scope ? (toValue(scope) ?? undefined) : undefined,
      );
   ...
   async function loadPlugin<K extends LoadablePlugin>(
      ...
      if (!plugin)
         throw new Error(
            `GSAP plugin "${name}" did not expose the expected export.`,
         );
